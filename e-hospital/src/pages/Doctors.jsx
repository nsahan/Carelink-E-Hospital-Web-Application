...
const Doctors = () => {
  ...
  const isDoctorAvailableToday = (doctor) => {
    ...
    if (doctor.offDays && doctor.offDays.includes(formattedDate)) {
      return false;
    }
  ...
  const handleReviewClick = (doctorId, e) => {
    e.stopPropagation();
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 py-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        {/* Filters Sidebar */}
        <div
          className={`md:w-1/4 ${isFilterOpen ? 'block' : 'hidden'} md:block bg-white rounded-xl shadow-sm p-6 h-fit transition-all duration-300`}
        >
              ...
              {specialities.map((spec, index) => (
                <label key={index} className="flex items-center space-x-3 cursor-pointer group">
                  <div className="relative flex items-center justify-center">
                    ...
                    />
                    <div
                      className={`w-5 h-5 border rounded transition-colors duration-200 ${selectedSpecialities.includes(spec)
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-3 flex items-center">
                      ...
                      <span className="ml-2 text-sm text-gray-500">
                        ({doctor.reviewCount || Math.floor(Math.random() * 100) + 20})
                      </span>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4">
                      <button
                        className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white py-2.5 px-4 rounded-lg font-medium transition duration-300"
                        onClick={() => handleDoctorClick(doctor._id)}
                      >
                        Book Appointment
                      </button>
                      <button
                        className="px-4 py-2.5 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition duration-300"
                        onClick={(e) => handleReviewClick(doctor._id, e)}
                      >
                        Add Review
                      </button>
                    </div>
